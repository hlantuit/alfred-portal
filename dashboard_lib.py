...
def format_marine_forecast_text(marine_entries, zone_name):
    """
...
def build_temperature_chart(lat, lon, now_utc, temp_cache, normal_years=30):
    ...
    ax.legend(loc="upper left", frameon=False, fontsize=10, labelcolor=NOTION_TEXT_GRAY)
...
def find_latest_sentinel1_date(token, lat, lon, site_label, lookback_days=10, half_width_km=150, now_utc=None):
    ...
    that actually covers the given site (not just anywhere in a broad
    search area) within the lookback window. Returns (date_str,
    full_datetime) or (None, None) if nothing was found / the search
    failed.

    site_label is used only for log messages (e.g. "covers Shingle Point
    with margin"), so logs from different sites are distinguishable.
    """
    if now_utc is None:
    ...
    try:
        ...
        print(f"SENTINEL-1: latest scene covering {site_label}: {latest_datetime}")
        return latest_datetime[:10], latest_datetime
...
def fetch_sentinel1_image(token, date_str, center_x, center_y, utm_epsg,
    ...
    given date, directly reprojected to the site's UTM zone — no
    separate rotation step needed here, since Sentinel Hub reprojects
    server-side (unlike GIBS/WMS for MODIS, which only serves
    north-up-at-its-own-central-meridian and needs the
    oversized-fetch-then-rotate workaround). UTM IS genuinely north-up
    at any longitude within its own zone, unlike EPSG:3413 — see the
    SENTINEL1_UTM_ZONE notes above the MODIS section for why the zone
    number itself must be computed per site.

    The dB conversion and 0-255 grayscale scaling are both done inside
    the evalscript (server-side), so the response is a ready-to-use
    8-bit PNG with alpha — avoiding any need for a GeoTIFF-reading
    library like rasterio/GDAL.
...
def fetch_and_process_sentinel1(lat, lon, site_label, utm_zone, utm_epsg,
    ...
    if sh_token:
        s1_date, s1_full_datetime = find_latest_sentinel1_date(sh_token, lat, lon, site_label, now_utc=now_utc)
        if s1_date:
            s1_raw = fetch_sentinel1_image(sh_token, s1_date, center_x, center_y, utm_epsg, half_width_m)
            if s1_raw:
                from PIL import Image
                import io as _io
                ...
                try:
                    ...
                    sentinel1_bytes = stamp_timestamp(sentinel1_bytes, to_local_time(s1_dt_utc, tz_name), label="Acquired")
                except Exception as e:
...
def fetch_copernicus_water_level(lat, lon, now_utc, site_label, yearly_mean=None):
    ...
    try:
        ...
        # rather than plain longitude/latitude.
...
def build_water_level_chart(times, values, tz_name, yearly_mean=None):
    ...
    try:
        ...
        # — a y-axis genuinely labeled this way communicates "typical"
        # more clearly than an absolute-meter axis plus a separately
        # labeled dashed reference line competing for space with the
        # curve.
        ...
        # Reference line at zero (= the yearly average) when plotting
        # relative values. Label uses axes-fraction coordinates (a fixed
        # spot near the top of the plot area) rather than the actual
        # data value 0 — the relative-to-mean curve regularly crosses
        # zero, including near the right edge where a data-coordinate
        # label would sit, which previously made the label overlap and
        # become unreadable against the curve.
        if yearly_mean is not None:
...
def build_hydrometric_chart(times, values_m, station_id, river_name):
    """
...
def build_header_blocks(now_local, logo_url=None, logo_png_bytes=None, institution_text=None):
    """
    ...
    blocks.append(paragraph(
        f"All times shown on this page are local time for this site "
...
def build_todays_conditions_section(weather_text, weather_source_text, weather_icon_block,
    ...
    for a site with no nearby IWLS tide station.
    """
    ...
    if tide_text is not None:
        ...
        blocks.append(columns(weather_card, wind_card))
        blocks.append(columns(sun_card))
...
def build_active_alerts_section(active_alerts):
    ...
    blocks = [
        ...
        callout(alert_lines, emoji="⚠️", color="yellow_background"),
    ]
...
def build_land_forecast_section(large_forecast_strip_bytes, land_forecast_caption):
    ...
    blocks = [
        ...
        callout("5-day outlook:", emoji="📅", color="purple_background", children=[block] if block else None),
...
def build_total_water_level_section(water_level_text, water_level_chart_bytes, water_level_chart_caption):
    ...
    if block:
        blocks.append(block)
    blocks.append(paragraph(fallback or water_level_chart_caption))
    blocks.append(divider())
    return blocks
...
def build_modis_section(modis_block, modis_caption, modis_date, now_utc, bbox_3413, site_display_name):
    ...
    blocks = [heading(f"🛰 Satellite View of {site_display_name}")]
    if modis_block:
...
def build_sentinel1_section(sentinel1_bytes, sentinel1_caption, sentinel1_explore_url, site_display_name):
    ...
    blocks = [heading(f"🛰 Radar View of {site_display_name}")]
    if block:
        blocks.append(block)
    blocks.append(paragraph(
        f"A radar image of {site_display_name}, which can see through cloud and darkness — useful when "
        f"the regular satellite photo above is blocked by weather. Gray areas were outside the "
        f"satellite's path that day."
    ))
    blocks.append(gray_caption(upload_fallback or sentinel1_caption))
    if sentinel1_explore_url:
        blocks.append(link_paragraph("Explore here →", sentinel1_explore_url))
    blocks.append(divider())
    return blocks
...
def build_tdd_histogram_section(tdd_histogram_bytes, tdd_histogram_caption):
    ...
    blocks = [heading("🌡 Thawing Degree Days — annual totals")]
    if block:
        blocks.append(block)
...
def build_wind_chart_section(wind_chart_bytes, wind_chart_caption):
    block, fallback = _upload_chart_or_caption(wind_chart_bytes, "wind_chart.png", "Wind vector chart could not be generated — see Action logs.")
...
def build_disclaimer_section(disclaimer_text=DEFAULT_DISCLAIMER_TEXT):
    return [disclaimer_paragraph(disclaimer_text)]
